...
void Unique::serialize(Archive &ar, const unsigned int version) {
}
...
template<class Archive>
void ModAction::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<Action>(*this);
    ar & mods;
}

template<class Archive>
void SendKey::load(Archive &ar, const unsigned int version) {
    guint code;
...
template<class Archive>
void SendKey::save(Archive &ar, const unsigned int version) const {
    guint code = 0;
    ar & boost::serialization::base_object<ModAction>(*this);
    ar & key;
    ar & code;
...
template<class Archive>
void SendText::load(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<Action>(*this);
    std::string str;
    ar & str;
    text = str;
...
template<class Archive>
void SendText::save(Archive &ar, const unsigned int version) const {
    ar & boost::serialization::base_object<Action>(*this);
    std::string str(text);
    ar & str;
}
...
template<class Archive>
void Scroll::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<ModAction>(*this);
}

template<class Archive>
void Ignore::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<ModAction>(*this);
}

template<class Archive>
void Button::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<ModAction>(*this);
    ar & button;
}

template<class Archive>
void Misc::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<Action>(*this);
    ar & type;
}

template<class Archive>
void StrokeSet::serialize(Archive &ar, const unsigned int version) {
    ar & boost::serialization::base_object<std::set<RStroke> >(*this);
}

template<class Archive>
void StrokeInfo::serialize(Archive &ar, const unsigned int version) {
    ar & strokes;
    ar & action;
    if (version == 0) return;
    ar & name;
...
const char *Misc::types[6] = {
    N_("None"), N_("Maximize"), N_("Unminimize"), N_("Show/Hide"), N_("Disable (Enable)"), nullptr
};

template<class Archive>
void ActionListDiff::serialize(Archive &ar, const unsigned int version) {
    ar & deleted;
    ar & added;
    ar & name;
    ar & children;
...
template<class Archive>
void ActionDB::load(Archive &ar, const unsigned int version) {
    if (version >= 2) {
        printf("Loading v2 actions.\n");
        ar & root;
    }
    if (version == 1) {
        printf("Loading v1 actions.\n");
        std::map<int, StrokeInfo> strokes;
        ar & strokes;
        for (auto &stroke: strokes)
            root.add(stroke.second);
    }
    if (version == 0) {
        printf("Loading v0 actions.\n");
        std::map<std::string, StrokeInfo> strokes;
        ar & strokes;
        for (auto &stroke: strokes) {
            stroke.second.name = stroke.first;
            root.add(stroke.second);
        }
    }
...
template<class Archive>
void ActionDB::save(Archive &ar, const unsigned int version) const {
    ar & root;
}
...
RStrokeInfo ActionListDiff::get_info(Unique *id, bool *deleted, bool *stroke, bool *name, bool *action) const {
    ...
    if (action)
        *action = false;
    RStrokeInfo si = parent ? parent->get_info(id) : boost::make_shared<StrokeInfo>();
...
boost::shared_ptr<std::map<Unique *, StrokeSet> > ActionListDiff::get_strokes() const {
    boost::shared_ptr<std::map<Unique *, StrokeSet> > strokes = parent
                                                                    ? parent->get_strokes()
                                                                    : boost::shared_ptr<std::map<Unique *, StrokeSet> >(
                                                                        new std::map<Unique *, StrokeSet>);
    for (auto i: deleted) {
        strokes->erase(i);
    }
...
boost::shared_ptr<std::set<Unique *> > ActionListDiff::get_ids(bool include_deleted) const {
    boost::shared_ptr<std::set<Unique *> > ids = parent
                                                     ? parent->get_ids(false)
...
void ActionListDiff::all_strokes(std::list<RStroke> &strokes) const {
    for (const auto &i: added) {
        for (const auto &stroke: i.second.strokes) {
...
RAction ActionListDiff::handle(RStroke s, RRanking &r) const {
    ...
    r->stroke = s;
    r->score = 0.0;
    const boost::shared_ptr<std::map<Unique *, StrokeSet> > strokes = get_strokes();
    for (std::map<Unique *, StrokeSet>::const_iterator i = strokes->begin(); i != strokes->end(); ++i) {
        for (auto j = i->second.begin(); j != i->second.end(); ++j) {
            double score;
            const int match = Stroke::compare(s, *j, score);
            if (match < 0) {
                continue;
            }
            const RStrokeInfo si = get_info(i->first);
...
void ActionListDiff::handle_advanced(RStroke s, std::map<guint, RAction> &as, std::map<guint, RRanking> &rs, int b1, int b2) const {
    if (!s) {
        return;
    }
    const boost::shared_ptr<std::map<Unique *, StrokeSet> > strokes = get_strokes();
        ...
        for (auto j = i->second.begin(); j != i->second.end(); ++j) {
            ...
            if (s->rockerLeft() || s->rockerRight()) {
                ...
                if (rs.count(b)) {
                    ...
                    r = new Ranking;
                    rs[b].reset(r);
                    r->stroke = boost::make_shared<Stroke>(*s);
                    r->score = -1;
                }
                const RStrokeInfo si = get_info(i->first);
                r->r.insert(pair<double, pair<std::string, RStroke> >
                    (score, pair<std::string, RStroke>(si->name, *j)));
                if (score > r->score && ((s->rockerLeft() && si->name == "Back") || (s->rockerRight() && si->name == "Forward"))) {
                    r->score = score;
                    r->name = si->name;
                    r->action = si->action;
                    r->best_stroke = *j;
