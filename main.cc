...
void icon_warning() {
    for (const auto &action: actions) {
        const auto m = dynamic_cast<Misc *>(action.second.action.get());
...
public:
    App(int &argc, char **&argv, const Glib::ustring &application_id,
        Gio::ApplicationFlags flags =
                Gio::APPLICATION_FLAGS_NONE) : Gtk::Application(argc, argv, application_id, flags), remote(false),
                                               enabled(nullptr) {
    }
...
void App::run_by_name(const char *str, const Glib::RefPtr<Gio::ApplicationCommandLine> &cmd_line) {
    for (const auto &action: actions) {
        if (action.second.name == std::string(str)) {
            if (action.second.action)
                xstate->queue(sigc::bind(sigc::mem_fun(xstate, &XState::run_action), action.second.action));
            return;
        }
    }
    char *msg;
...
void App::on_activate() {
    ...
    const Glib::RefPtr<Gdk::Screen> screen = Gdk::Display::get_default()->get_default_screen();
    ...
    const auto trace_notify = new Notifier(sigc::ptr_fun(&schedule_reload_trace));
    prefs.trace.connect(trace_notify);
    prefs.color.connect(trace_notify);

    XTestGrabControl(dpy, True);

    const Glib::RefPtr<Glib::IOSource> io = Glib::IOSource::create(ConnectionNumber(dpy), Glib::IO_IN);
...
void SendKey::run() {
    if (!key)
        return;
    const guint code = XKeysymToKeycode(dpy, key);
...
class Modifiers : Timeout {
    ...
    static void update_mods() {
        ...
        for (const auto i: all) {
            new_state |= static_cast<int>(i->mods);
        }
        for (int i = 0; i < n_modkeys; i++) {
            const int mask = static_cast<int>(modkeys[i].mask);
...
public:
    ...
    bool operator==(const Modifiers &m) const {
...
void Misc::run() {
    switch (type) {
        case SHOWHIDE:
            ...
            return;
        case MAXIMIZE: {
            const auto w = current_app_window.get();
            if (w) {
...
void get_full_workarea(GdkRectangle *full_area) {
    ...
    const int num_monitors = gdk_display_get_n_monitors(display);

    constexpr GdkRectangle empty1 = {0, 0, 0, 0};
    constexpr GdkRectangle empty2 = {0, 0, 0, 0};
    gdk_rectangle_union(&empty1, &empty2, full_area);
    for (int i = 0; i < num_monitors; i++) {
        GdkMonitor *monitor = gdk_display_get_monitor(display, i);
        GdkRectangle workarea;
        gdk_monitor_get_workarea(monitor, &workarea);

        gdk_rectangle_union(full_area, &workarea, full_area);
    }
}

void maximize_window(Display *display, const Window win) {
    ...
    xev.format = 32;
    xev.data.l[0] = 1; // 1 to enable, 0 to remove, 2 to toggle
    xev.data.l[1] = static_cast<long>(max_horz);
    xev.data.l[2] = static_cast<long>(max_vert);
    xev.data.l[3] = 0;
    xev.data.l[4] = 0;
